# Phase 8 — Quote / Pricing Engine (journey Step 8)
...
- Read `docs/PHASE-7.md` (previous phase doc — journey Step 7, Alternatives).
...
**Phase-numbering reconciliation (same discipline `docs/PHASE-6.md`/`docs/PHASE-7.md` §1 used —
...
- Location/distance-based delivery pricing — `deliveryFee` is a flat rate; the fleet catalog has no
  branch/location field yet (same gap `docs/PHASE-7.md` §9 already noted).
...
- **`Money`: integer minor units (fils), two controlled float boundaries, both documented and
  ...
  minor units" gap `docs/PHASE-7.md` §9 flagged as inherited from Phase 3 — fixed here for all of
...
- `docs/PHASE-8.md` (this file), `docs/PHASE-CONTRACTS.json` (id 14, see §1).
...
| Architecture review | checklist vs `MASTER-PLAN.md` §1, §6 | ✅ matches target layering; this phase's own money handling now satisfies §6's "integer minor units, never floats" (closing the gap `docs/PHASE-7.md` §9 flagged as inherited from Phase 3)               |
...
- **Flat delivery fee, no location/distance pricing** — the fleet catalog has no branch/location
  field yet (same gap `docs/PHASE-7.md` §9 noted).
...
Per `docs/PHASE-7.md` §13 and `MASTER-PLAN.md` §4, several threads remain open, none started here:
